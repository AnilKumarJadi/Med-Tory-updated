...
export const ProductContextProvider = ({ children }) => {
  ...
  const getProductData = async () => {
    try {
      const apiResponse = await axios.get("http://localhost:4001/product");
      setApiData(apiResponse.data);
      // console.log(apiData)

      const approvalResponse = await axios.get(
        "http://localhost:4001/postproduct"
      );
      setApprovalData(approvalResponse.data);

      const rejectedResponse = await axios.get(
        "http://localhost:4001/RejectedData"
      );
      setRejectedData(rejectedResponse.data);

      // supplier purchase order statuspage

const approvedserverData=await axios.get('http://localhost:4001/ApprovalData');
...
// console.log(approvedData)
  ...
  const handleSubmit = async (e) => {
    ...
    if (productData.product && productData.quantity) {
      try {
        ...
        await axios.post("http://localhost:4001/postproduct", productData);
        Swal.fire({
          title: "Success",
  ...
  const handleApprovalbtn = async (approvedItem, id) => {
    try {
      ...
      await axios.post("http://localhost:4001/ApprovalData", approvedItem);
      ...
      await axios.delete(`http://localhost:4001/postproduct/${id}`);
    } catch (error) {
  ...
  const handleRejectbtn = async (rejectedItem, id) => {
    try {
      ...
      await axios.post("http://localhost:4001/RejectedData", rejectedItem);
